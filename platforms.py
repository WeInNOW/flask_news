...
def index():
    ...
    context = {
        'articles': RsAtricleInfo.query.order_by(RsAtricleInfo.update_date.desc()).limit(10),
...
def recommender():
    ...
    context = {
        'articles': RsAtricleInfo.query.filter(RsAtricleInfo.article_id.in_(impressions)).order_by(RsAtricleInfo.update_date),
        'name': '推荐'
    }
...
def detail(article_id):
    ...
    article_model = RsAtricleInfo.query.filter(RsAtricleInfo.article_id == article_id).first()
    return render_template('detail.html', article=article_model,
                           relate_articles=[article_model, article_model],)
